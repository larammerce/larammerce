...
/**
 *
 * @method static LanguageSettingData getRecord($name)
 */
class LanguageConfig extends BaseFeatureConfig
{
    ...
    public static function defaultRecord($name): LanguageSettingData
    {
        if ($name == config("translation.fallback_locale")) {
            return new LanguageSettingData($name, true, true);
        }
        return new LanguageSettingData($name, false, false);
    }

    /**
     * @throws NotValidSettingRecordException
     */
    public static function updateRecord(string $lang_id, bool $is_enabled, bool $is_default): LanguageSettingData
    {
        $language_model = new LanguageSettingData($lang_id, $is_enabled, $is_default);
        LanguageConfig::setRecord($language_model);
    ...
    public static function setAll(array $languages): void
    {
        ...
        foreach ($languages as $lang_id => $lang_config_data) {
            if (in_array($lang_id, config("translation.available_locales"))) {
                self::$CACHED_DATA[$lang_id] = LanguageConfig::updateRecord(
