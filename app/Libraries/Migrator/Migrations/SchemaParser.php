...
namespace App\Libraries\Migrator\Migrations;
...
class SchemaParser
{
    ...
    {
        $reference_table = $this->getTableNameFromForeignKey($segments['name']);
        $key_name = $segments['name'];
        $foreign_name = implode("", array_map(function ($part) {
                return Str::limit($part, 2, "");
            }, explode("_", $key_name))) . "_" . $reference_table . "_fr";
        $string = sprintf(
            "%s:foreign('%s'):references('id'):on('%s'):onDelete('cascade')",
            $key_name,
            $foreign_name,
            $reference_table
        );
