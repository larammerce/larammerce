...
use App\Libraries\Migrator\GeneratorException;
...
class SyntaxBuilder
{
     ...
     * @param array $meta
     * @return string
     * @throws \App\Libraries\Migrator\GeneratorException
     */
    ...
    {
        $name = str_starts_with($field["name"], "[") ? $field["name"] : "'{$field["name"]}'";
        $syntax = sprintf("\$table->%s(%s)", $field['type'],$name);
    ...
    {
        $name = str_starts_with($field["name"], "[") ? $field["name"] : "'{$field["name"]}'";
        return sprintf("\$table->dropColumn(%s);", $name);
    }
}
