(function (factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define('fastsearch', ['jquery'], factory);
...
(function (root, factory) {
    ...
    if (typeof define === 'function' && define.amd) {
        define('fastselect', ['jquery', 'fastsearch'], factory);
    } else if (typeof module === 'object' && module.exports) {
    ...
    $.extend(Fastselect.prototype, {
        ...
        setupSingleElement: function (onDone) {
            ...
            this.$queryInput = $('<input>').attr('placeholder', this.options.searchPlaceholder).addClass(this.options.queryInputClass).addClass(this.options.formControlClass).appendTo(this.$controls);
        ...
        setupMultipleElement: function (onDone) {
            ...
            this.$queryInput = $('<input>').addClass(options.queryInputClass).addClass(this.options.formControlClass).appendTo(this.$controls);
        ...
        processInitialOptions: function () {
            ...
            if (this.hasCustomLoader) {
                ...
                options = $.map(this.$input.find('option:selected').get(), function (option) {
                    ...
                    var $option = $(option);
                    var jsonData = ($option.data('json') || {});
                    return jQuery.merge(jsonData, {
                        text: $option.text(),
        ...
        addChoiceItem: function (optionModel) {
            $(
                '<div data-json="' + JSON.stringify(optionModel).split('"').join("'") + '" data-text="' + optionModel.text + '" data-value="' + optionModel.value + '" class="' + this.options.choiceItemClass + '">' +
        ...
        setSelectedOption: function (option) {
            ...
            selectedModel && this.$input.trigger('model:attach', selectedModel);
            this.updateDomElements();
        ...
        removeSelectedOption: function (option, $choiceItem) {
            ...
            if (removedModel && removedModel.$item) {
                ...
                this.$input.trigger('model:detach', removedModel);
            }

            if ($choiceItem) {
                var data = (jQuery.parseJSON(option.split("'").join('"')));
                this.$input.trigger('model:detach', data);
                $choiceItem.remove();
            } else {
        ...
        gleanOptionsData: function ($options) {
            ...
            return $.map($options.get(), function (option) {
                var $option = $(option);
                return jQuery.merge(($option.data('json') || {}), {
                    text: $option.text(),
                    value: $option.attr('value'),
    ...
    Fastselect.defaults = {
        ...
        noResultsText: 'هیچ داده ای یافت نشد',
        userOptionPrefix: 'اضافه کردن '
