@foreach($invoice_rows as $invoice_row)
    <div
        class="col-lg-offset-1 col-lg-10 col-md-offset-0 col-md-12 col-sm-offset-0 col-sm-12 col-xs-offset-0 col-xs-12 list-row roles">
        <div class="col-lg-1 col-md-1 col-sm-2 col-xs-3 col">
            <div class="img-container">
                <img class="img-responsive" src="{{ ImageService::getImage($invoice_row->product, 'thumb') }}">
            </div>
        </div>
        <div class="col-lg-1 col-md-1 col-sm-2 col-xs-3 col">
            <div class="label">شناسه</div>
            <div>{{$invoice_row->id}}#</div>
        </div>
        <div class="col-lg-3 col-md-2 col-sm-4 col-xs-12 col">
            <div class="label">نام محصول</div>
            <div>{{$invoice_row->product->title}}</div>
        </div>
        <div class="col-lg-1 col-md-1 col-sm-2 col-xs-6 col">
            <div class="label">تعداد</div>
            <div>{{$invoice_row->count}}</div>
        </div>
        <div class="col-lg-1 col-md-1 col-sm-2 col-xs-12 col">
            <div class="label">تخفیف</div>
            <div>{{$invoice_row->discount_percentage}}%</div>
        </div>
        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 col">
            <div class="label">قیمت واحد</div>
            <div class="price-data">{{$invoice_row->paymentPrice() }}</div>
        </div>
        <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12 col">
            <div class="label">مجموع قیمت</div>
            <div class="price-data">{{$invoice_row->sum() }}</div>
        </div>
        <div class="col-lg-1 col-md-2 col-sm-4 col-xs-12 col">
            <div class="label">عملیات</div>
            <div class="actions-container">
                <a class="btn btn-sm btn-primary" href="#" disabled>
                    <i class="fa fa-pencil"></i>
                </a>
                <a class="btn btn-sm btn-danger" href="#" disabled confirm>
                    <i class="fa fa-trash"></i>
                </a>
            </div>
        </div>
    </div>
@endforeach
