@foreach($attribute_values as $attribute_value)
    <div
        ...
        <div class="col-lg-1 col-md-2 col-sm-2 col-xs-3 col">
            ...
            <div>{{$attribute_value->id}}#</div>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-4 col-xs-6 col">
            <div class="label">نام کلید مربوطه</div>
            <div>{{$attribute_value->key->title}}</div>
        </div>
        <div class="col-lg-3 col-md-3 col-sm-4 col-xs-6 col">
            <div class="label">مقدار</div>
            <div>{{$attribute_value->name}}</div>
        </div>
        <div class="col-lg-4 col-md-9 col-sm-7 col-xs-12 col">
            <div class="label">عملیات</div>
            <div class="actions-container">
                <a class="btn btn-sm btn-primary"
                   href="{{route('admin.p-structure-attr-value.edit', $attribute_value)}}">
                    <i class="fa fa-pencil"></i>
                </a>
                <a class="btn btn-sm btn-danger virt-form"
                   data-action="{{ route('admin.p-structure-attr-value.destroy', $attribute_value) }}"
                   data-method="DELETE" confirm>
                    <i class="fa fa-trash"></i>
                </a>
                <a class="btn btn-sm btn-success"
                   href="{{route('admin.p-structure-attr-value.show', $attribute_value)}}">
                    <i class="fa fa-eye"></i>
                </a>
            </div>
        </div>
