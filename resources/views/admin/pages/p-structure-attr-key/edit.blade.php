...
@section('bread_crumb')
    ...
    <li class="active"><a href="{{route('admin.p-structure-attr-key.index', $p_structure_attr_key)}}">ویرایش ويژگی</a></li>

@endsection

@section('form_title')ویرایش ویژگی محصول@endsection

@section('form_attributes') action="{{route('admin.p-structure-attr-key.update', $p_structure_attr_key)}}" method="POST" form-with-hidden-checkboxes @endsection

@section('form_body')

    {{ method_field('PUT') }}
    <input type="hidden" name="id" value="{{ $p_structure_attr_key->id }}">
    <div class="input-group group-sm col-lg-12 col-sm-12 col-md-12 col-xs-12">
        <span class="label">عنوان ویژگی</span>
        <input class="form-control input-sm" name="title" value="{{ $p_structure_attr_key->title }}">
    </div>
    <div class="input-group group-sm col-lg-12 col-sm-12 col-md-12 col-xs-12">
        <span class="label">سطح اولویت</span>
        <input class="form-control input-sm" name="priority" value="{{$p_structure_attr_key->priority}}">
    </div>
    <div class="input-group filled group-sm col-lg-12 col-sm-12 col-md-12 col-xs-12">
        ...
        <select name="show_type" class="form-control input-sm">
            @foreach(PSAttrKeyShowType::toMap() as $key => $value )
                <option value="{{$value}}" @if($p_structure_attr_key->show_type == $value) selected @endif>
         ...
         style="margin-bottom: 40px;">
            <span class="material-switch pull-right">آپشن محصولات ؟ &nbsp
                <input id="is_model_option" name="is_model_option" type="checkbox" value="1"
                       @if($p_structure_attr_key->is_model_option) checked @endif/>
         ...
         style="margin-bottom: 40px;">
            <span class="material-switch pull-right">قابل مرتب سازی ؟ &nbsp
                <input id="is_sortable" name="is_sortable" type="checkbox" value="1"
                       @if($p_structure_attr_key->is_sortable) checked @endif/>
    ...
    <div class="col-lg-12 col-sm-12 col-md-12 col-xs-12 input-group tag-manager">
        <div class="tags-container nabs">
            <ul tag-input-name="<%- name %>">
                @foreach($p_structure_attr_key->values()->orderBy('priority')->get() as $value)
                ...
                <li class="action btn btn-sm btn-success" act="link"
                    href="{{route('admin.p-structure-attr-value.create')}}?id={{ $p_structure_attr_key->id }}">
...
@section('form_footer')
    ...
    <a type="submit" class="btn btn-info btn-sm"
       href="{{route("admin.p-structure-attr-value.index")}}?p_structure_attr_key_id={{$p_structure_attr_key->id}}">نمایش لیست
        مقادیر</a>
@endsection
